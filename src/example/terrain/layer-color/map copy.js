...
const float ARTIFACT_SUPPRESSION = 1.0;
...
float lambertFromNormal(vec3 normalEC, vec3 lightEC) {
    return max(dot(normalize(normalEC), normalize(lightEC)), 0.0);
}
...
float normalHillshade(vec3 normalEC, float slope) {
    ...
    return mix(saturate(softShade), 1.0, 0.18);
}
...
float lineArtifactMask(float geographicShade, float slope, float aspect, float heightT) {
    ...
    float edgeSignal = shadeEdge * 0.95
                     + slopeEdge * 0.35
                     + aspectEdge * 0.65
                     + heightEdge * 1.2;
    float thinJump = smoothstep(0.012, 0.075, edgeSignal);
    float broadLandform = smoothstep(0.055, 0.22, heightEdge);
    return saturate(thinJump * (1.0 - broadLandform * 0.45) * ARTIFACT_SUPPRESSION);
#else
    return 0.0;
#endif
}
