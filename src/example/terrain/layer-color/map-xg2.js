...
const TERRAIN_URL =
  "/terrain-api/tiles/dynamic?cog=%2Fhome%2Fshilei%2Fsource-data%2Ftiff%2FGF7_DLC_E110.6_N32.7_20250111_L1A0001829881_StereoPair%2FGF7_DLC_E110.6_N32.7_20250111_DEM0001829881-BWDPAN.tif";

const SAND_TABLE_STYLE = {
  backgroundColor: "#f4ecd8",
  baseColor: "#f4ecd8",
  clippingEdgeColor: "#111111",
...
async function loadTerrain() {
  try {
    ...
    viewer.scene.globe.material = createTerrainMaterial({
      ...
      artifactSuppression: SAND_TABLE_STYLE.artifactSuppression,
...
float geographicHillshade(float slope, float aspect) {
...
float globalMapperHillshade(float slope, float aspect) {
    return geographicHillshade(slope, aspect);
}

float lambertFromNormal(vec3 normalEC, vec3 lightEC) {
    return max(dot(normalize(normalEC), lightEC), 0.0);
}

float normalHillshade(vec3 normalEC, float slope) {
    vec3 normal = normalize(normalEC);
    vec3 primaryLight = normalize(czm_lightDirectionEC);
    vec3 fillLight = normalize(primaryLight + vec3(0.35, 0.2, 0.45));
    vec3 backLight = normalize(vec3(-primaryLight.x, -primaryLight.y, max(primaryLight.z, 0.25)));

    float primary = lambertFromNormal(normal, primaryLight);
    float fill = lambertFromNormal(normal, fillLight);
    float back = lambertFromNormal(normal, backLight);
    float slopeTerm = smoothstep(0.04, 1.15, slope);

    float softShade = primary * 0.72 + fill * 0.18 + back * 0.1;
    softShade = mix(1.0, softShade, slopeTerm);
    softShade = mix(0.5, softShade, shadeStrength);
    softShade = pow(saturate(softShade), max(contrast, 0.01));
    softShade = mix(saturate(softShade), 1.0, 0.18);

    float crispShade = primary * 0.86 + fill * 0.1 + back * 0.04;
    crispShade = mix(1.0, crispShade, slopeTerm);
    crispShade = pow(saturate(crispShade * shadeStrength), max(contrast, 0.01));
    crispShade = smoothstep(shadowCutoff, highlightCutoff, crispShade);
    crispShade = pow(saturate(crispShade), atlasShadowPower);

    return mix(softShade, crispShade, 0.7);
}

float lineArtifactMask(float geographicShade, float slope, float aspect, float heightT) {
#if (__VERSION__ == 300 || defined(GL_OES_standard_derivatives))
    float shadeEdge = fwidth(geographicShade);
    float slopeEdge = fwidth(slope) / PI;
    vec2 aspectVector = vec2(cos(aspect), sin(aspect));
    float aspectEdge = length(fwidth(aspectVector));
    float heightEdge = fwidth(heightT);
    float thinDirectionalChange = smoothstep(
        0.012,
        0.075,
        shadeEdge * 0.95 + slopeEdge * 0.35 + aspectEdge * 0.65
    );
    float broadLandform = smoothstep(0.045, 0.18, heightEdge);
    return saturate(thinDirectionalChange * (1.0 - broadLandform * 0.55) * artifactSuppression);
#else
    return 0.0;
#endif
}

float suppressLineArtifacts(float shade, float artifactMask) {
    float cappedShade = clamp(shade, shadowCutoff, highlightCutoff);
    return mix(shade, cappedShade, artifactMask * 0.55);
}
...
vec3 applyGlobalMapperLighting(vec3 color, float shade, float slope, float aspect, float artifactMask) {
    float artifactBlend = saturate(artifactMask);
    float slopeRelief = smoothstep(0.035, 0.95, slope);
    float shadowBlend = hillshadeBlend * slopeRelief * mix(0.92, 0.58, artifactBlend);
    float shadow = mix(1.0, shade, shadowBlend);
    float facing = aspectFacingLight(aspect);
    float valley = max(-facing, 0.0) * slopeRelief;
    valley *= 1.0 - artifactBlend * 0.82;

    vec3 lit = color * max(ambient, shadow) * baseBrightness;
    lit *= 1.0 - valley * slopeDarkness * mix(0.82, 0.24, artifactBlend) * (1.0 - shade);
    lit *= 1.0 - slopeRelief * slopeDarkness * mix(0.12, 0.04, artifactBlend);
...
czm_material czm_getMaterial(czm_materialInput materialInput) {
    ...
    float geographicShade = globalMapperHillshade(slope, aspect);
    float fallbackShade = normalHillshade(normalEC, slope);
    float artifactMask = lineArtifactMask(geographicShade, slope, aspect, t);
    float shade = mix(geographicShade, fallbackShade, artifactMask * 0.82);
    shade = suppressLineArtifacts(shade, artifactMask);

    color = applyGlobalMapperLighting(color, shade, slope, aspect, artifactMask);
